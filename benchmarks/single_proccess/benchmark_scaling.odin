...
generate_scaling_configs :: proc(cpu_count: int) -> []shared.Scaling_Config {
	...
	contention_per_sender := 200_000

	if cpu_count >= 8 {
		sc := min(32, cpu_count * 2)
		append(
			&configs,
			shared.Scaling_Config {
				actor_count = 1,
				sender_count = sc,
				message_count = sc * contention_per_sender,
				message_size = .Empty,
				description = fmt.tprintf("%d:1 Empty", sc),
				category = .CONTENTION,
				dedicated = false,
			},
		)
	...
	if cpu_count >= 16 {
		append(
			...
			shared.Scaling_Config {
				actor_count = 1,
				sender_count = 64,
				message_count = 64 * contention_per_sender,
				message_size = .Empty,
...
run_scaling_benchmark :: proc() {
	...
	fmt.println(
		"Test                 │ Category │  Msgs/sec  │    MB/s  │ Spread     │ Errors",
	)
	fmt.println(
		"─────────────────────┼──────────┼────────────┼──────────┼────────────┼────────",
	)
	...
	for config in configs {
		...
		bandwidth := (median_tput * f64(bytes)) / (1024 * 1024)
		...
		window_s := f64(config.message_count) / median_tput if median_tput > 0 else 0
		if window_s < 0.010 {
			flag := fmt.tprintf("SHORT %.0fms", window_s * 1000)
			error_str = flag if error_str == "-" else fmt.tprintf("%s %s", error_str, flag)
		}
