...
test_hot_reload_basic :: proc(t: ^testing.T) {
	...
	for _ in 0 ..< 3 {
		...
		expect(t, err == .OK, "Failed to send message")
	}

	expect(t, hr_wait_for_count(pid, 3), "v1 should have incremented to 3")

	v2_path, tmp, built := hr_build_test_module("counter_v2")
	defer os.remove_all(tmp)
	expect(t, built, "Failed to build counter_v2.so")
	if !built do return
	...
	for _ in 0 ..< 2 {
		err := actod.send_message(pid, "tick")
		expect(t, err == .OK, "Failed to send message after reload")
	}

	expect(t, hr_wait_for_count(pid, 23), "v2 should have incremented to 23")

	count, ok := hr_read_count(pid)
	expect(t, ok, "should be able to read count")
	expect_value(t, count, i32(23))
}
...
test_hot_reload_state_preserved :: proc(t: ^testing.T) {
	...
	defer os.remove_all(tmp)
	if !built {
		expect(t, false, "Failed to build counter_v2.so")
		return
	}
...
test_rollback :: proc(t: ^testing.T) {
	...
	if !v1_built {
		expect(t, false, "Failed to build counter_v1.so")
		return
	}
...
test_file_watcher_detection :: proc(t: ^testing.T) {
	...
	if detected {
		...
		expect_value(t, name, "test_actor")
	}
}
...
test_hot_reload_under_load :: proc(t: ^testing.T) {
	...
	expect_value(t, count_after, count_before + 10)
}
