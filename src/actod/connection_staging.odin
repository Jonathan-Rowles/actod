package actod

import "base:intrinsics"
import "core:sync"

ACTOD_NET_STAGING :: #config(ACTOD_NET_STAGING, true)

STAGING_DESTS :: 4
STAGING_BUF_SIZE :: 8192
STAGE_FRAME_MAX :: 1024

Staging_Entry :: struct {
	ring: ^Connection_Ring,
	len:  u32,
	cap:  u32,
	buf:  [STAGING_BUF_SIZE]byte,
}

Net_Staging :: struct {
	entries: [STAGING_DESTS]Staging_Entry,
	active:  u32,
}

@(thread_local)
net_staging: Net_Staging

staging_reserve :: proc(ring: ^Connection_Ring, size: u32) -> (dst: []byte, ok: bool) {
	entry := staging_entry_for(ring)
	if entry == nil || size > entry.cap {
		return nil, false
	}
	if entry.len + size > entry.cap {
		if !staging_flush_entry(entry) {
			return nil, false
		}
		entry.ring = ring
		entry.cap = staging_entry_cap(ring)
		net_staging.active += 1
	}
	offset := entry.len
	entry.len += size
	return entry.buf[offset:offset + size], true
}

staging_unreserve :: proc(ring: ^Connection_Ring, size: u32) {
	for &entry in net_staging.entries {
		if entry.ring == ring && entry.len >= size {
			entry.len -= size
			if entry.len == 0 {
				entry.ring = nil
				net_staging.active -= 1
			}
			return
		}
	}
}

staging_flush_ring :: proc(ring: ^Connection_Ring) -> bool {
	if net_staging.active == 0 {
		return true
	}
	for &entry in net_staging.entries {
		if entry.ring == ring {
			return staging_flush_entry(&entry)
		}
	}
	return true
}

staging_flush_all :: proc() -> bool {
	if net_staging.active == 0 {
		return true
	}
	all_clear := true
	for &entry in net_staging.entries {
		if entry.ring != nil && !staging_flush_entry(&entry) {
			all_clear = false
		}
	}
	return all_clear
}

staging_has_pending :: #force_inline proc() -> bool {
	return net_staging.active != 0
}

@(private = "file")
staging_entry_cap :: proc(ring: ^Connection_Ring) -> u32 {
	return min(u32(STAGING_BUF_SIZE), ring.usable_slot_size)
}

@(private = "file")
staging_entry_for :: proc(ring: ^Connection_Ring) -> ^Staging_Entry {
	free_entry: ^Staging_Entry
	for &entry in net_staging.entries {
		if entry.ring == ring {
			return &entry
		}
		if entry.ring == nil && free_entry == nil {
			free_entry = &entry
		}
	}
	if free_entry == nil {
		free_entry = &net_staging.entries[0]
		for &entry in net_staging.entries {
			if entry.len > free_entry.len {
				free_entry = &entry
			}
		}
		if !staging_flush_entry(free_entry) {
			return nil
		}
	}
	free_entry.ring = ring
	free_entry.len = 0
	free_entry.cap = staging_entry_cap(ring)
	net_staging.active += 1
	return free_entry
}

@(private = "file")
staging_flush_entry :: proc(entry: ^Staging_Entry) -> bool {
	if entry.ring == nil {
		return true
	}
	if entry.len == 0 {
		entry.ring = nil
		net_staging.active -= 1
		return true
	}

	ring := entry.ring
	if sync.atomic_load(&ring.park_state) != .Active {
		if fresh := get_connection_ring(ring.node_id); fresh != nil {
			ring = fresh
		}
	}

	if !batch_append_blob(ring, entry.buf[:entry.len]) {
		return false
	}
	entry.len = 0
	entry.ring = nil
	net_staging.active -= 1
	return true
}

staging_drop_node_rings :: proc() {
	if net_staging.active == 0 {
		return
	}
	for &entry in net_staging.entries {
		ring := entry.ring
		if ring == nil {
			continue
		}
		if !node_owns_ring(ring) {
			continue
		}
		entry.ring = nil
		entry.len = 0
		net_staging.active -= 1
	}
}

@(private = "file")
node_owns_ring :: proc(ring: ^Connection_Ring) -> bool {
	nid := ring.node_id
	if nid >= MAX_NODES {
		return false
	}
	if NODE.connection_rings[nid] == ring {
		return true
	}
	pool := NODE.connection_pools[nid]
	if pool == nil {
		return false
	}
	for i in 0 ..< MAX_POOL_RINGS {
		if pool.rings[i] == ring || pool.parked[i] == ring {
			return true
		}
	}
	return false
}

staging_flush_before_park :: proc() -> bool {
	if net_staging.active == 0 {
		return true
	}
	for attempt := 0; attempt < 64; attempt += 1 {
		if staging_flush_all() {
			return true
		}
		intrinsics.cpu_relax()
	}
	return false
}
