...
While it's running, edit the `// <- change this` lines in the actor files under `hot_reload_actors/` and save. The runtime will detect the change, recompile, and swap the behaviour. You will see the new log messages appear without restarting.
